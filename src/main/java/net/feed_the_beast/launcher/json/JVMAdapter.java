...
public class JVMAdapter implements JsonDeserializer<JVM> {
    @Override
    public JVM deserialize (JsonElement json, Type type, JsonDeserializationContext context)
            throws JsonParseException {
        List<String> values = Lists.newArrayList();
        List<OSRule> gamerules = Lists.newArrayList();
        boolean use = false;
        String textstr = null;
        ...
        if (rules != null) {
            if (rules.isJsonArray()) {
                for (JsonElement e : rules.getAsJsonArray()) {
                    gamerules.add(JsonFactory.GSON.fromJson(e.getAsJsonObject(), OSRule.class));
                }
            }
        ...
        return new JVM(values, gamerules, textstr, use);
    }
}